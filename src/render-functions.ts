...
function drawColorBar(
    ...
    for (let pos = x; pos < x + width; pos += 1) {
...
export function renderHealthBar(
    ...
    drawColorBar(display, 0, 45, totalWidth, Colors.BarEmpty);
    drawColorBar(display, 0, 45, barWidth, Colors.BarFilled);

    const healthText = `HP:  ${currentValue}/${maxValue}`;

    for (let i = 0; i < healthText.length; i += 1) {
...
export function renderFrameWithTitle(
    ...
    window
        ...
        .display
        .drawText(x, y, topRow);
    for (let i = 1; i < innerHeight; i += 1) {
        window
            .engine
            .display
            .drawText(x, y + i, middleRow);
    }

    window
        .engine
        .display
        .drawText(x, y + height - 1, bottomRow);
}