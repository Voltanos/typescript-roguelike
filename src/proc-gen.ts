...
function getMaxValueForFloor(
    ...
    for (const [min, value] of maxValueByFloor) {
        if (min > floor) break;
        current = value;
    }
...
function placeEntities(
    ...
    const numberOfMonstersToAdd: number = generateRandomNumber(
        0,
...
export function generateDungeon(
    ...
    for (let count = 0; count < maxRooms; count += 1) {
    ...
    // Set downstairs to the center of the last room.
    dungeon
        .tiles[centerOfLastRoom[1]][centerOfLastRoom[0]] = {
        ...STAIRS_DOWN_TILE
    };
...
function getWeights(
    ...
    const current: WeightMap = {};

    for (const { floor, weights } of chancesByFloor) {
